...
ACTOR_LEARNING_RATE = 0.001 # hidden layer 5
CRITIC_LEARNING_RATE = 0.004 # hidden layer 5
...
def train(agent, env, sess, num_episodes=NUM_EPISODES):
  ...
  for i in range(NUM_EPISODES):
    ...
    if i % TRAIN_EVERY_NUM_EPISODES == 0 and not (i > 20 and sum(history[-10:]) >= 10*(MAX_STEPS-10)):
      print('train at episode {}'.format(i))
      td_error = agent.learn(episode, sess)
      print("\tTD_ERROR: {}".format(td_error))
  return agent, history


agent = complete.PolicyGradientNNAgent(actor_lr=ACTOR_LEARNING_RATE,
